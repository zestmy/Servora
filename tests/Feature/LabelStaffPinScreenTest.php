...
use App\Livewire\Labels\Staff\Pin;
...
/**
 * The Label PWA's account screen answers "who is signed in" — and a face
 * answers that faster than a name. The avatar is the shared staff-avatar
 * component: photo over initials, so an employee with no photo on file
 * (most of the floor) still gets their coloured disc, and the photo is
 * served by the PIN-session route rather than the manager's hr.view one.
 */
class LabelStaffPinScreenTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function setUp(): void
    {
        ...
        $company = Company::create([
            'name'      => 'Pin Screen Co',
            'slug'      => Str::slug('Pin Screen Co') . '-' . uniqid(),
            'currency'  => 'MYR',
            'is_active' => true,
        ]);
    ...
    public function test_the_account_screen_shows_the_photo_when_there_is_one(): void
    {
        ...
        Livewire::test(Pin::class)
            ...
            // The employee's own face, from the PIN-session photo route.
