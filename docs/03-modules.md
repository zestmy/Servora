...
| [Admin/Invoices/Index](../app/Livewire/Admin/Invoices/Index.php) | `/admin/invoices` | SUBSCRIPTION billing ledger — the platform billing its tenants. Not to be confused with `purchasing.invoices.*`, which is a tenant recording what its supplier billed it. Issue / settle / void; KPIs are deliberately unfiltered. |
| [Admin/Invoices/Form](../app/Livewire/Admin/Invoices/Form.php) | `/admin/invoices/create`, `/admin/invoices/{id}/edit` | Raise or amend a **draft** invoice. Anything issued redirects out — see `InvoiceService::updateDraft()`. |
| [Admin/BillingSettings](../app/Livewire/Admin/BillingSettings.php) | `/admin/billing-settings` | Seller details and invoice defaults, stored as `AppSetting` rows so a company address change is not a deploy. |
| [Admin/Docs/Index](../app/Livewire/Admin/Docs/Index.php) | `/admin/docs` | Help-centre sections and articles: publish, reorder, delete. |
| [Admin/Docs/ArticleForm](../app/Livewire/Admin/Docs/ArticleForm.php) | `/admin/docs/articles/create`, `/admin/docs/articles/{id}/edit` | Write one article (HTML body, write/preview) and upload its figures. |
...
## Help Centre (`/help`, public)

| Component | Route | Purpose |
|-----------|-------|---------|
| [Help/Index](../app/Livewire/Help/Index.php) | `/help` | Section tiles plus a LIKE search across title, excerpt, keywords and body. |
| [Help/Category](../app/Livewire/Help/Category.php) | `/help/{categorySlug}` | One section's published articles. |
| [Help/Article](../app/Livewire/Help/Article.php) | `/help/{categorySlug}/{articleSlug}` | One article, with in-section contents and prev/next. |

Outside every auth group on purpose — most of what the manual answers is asked
before anyone has an account. Rendered in `layouts.marketing` for both guests
and signed-in users.

**The route parameters are `{categorySlug}` / `{articleSlug}`, never
`{category}` / `{article}`.** Livewire assigns a route parameter onto the
public property of the same name *before* `mount()` runs, so a `{category}`
segment tries to store a string in a property typed `DocCategory` and 404s
every article in the manual while the routes and the rows are both correct.
`HelpCentreTest` pins the route URI.

Content is seeded by [DocsSeeder](../database/seeders/DocsSeeder.php) from one
file per section in `database/seeders/docs/`, and edited afterwards at
`/admin/docs`. The seeder upserts by slug and never overwrites an existing
row — reseeding adds what is missing and leaves an editor's corrections alone.

Figures are generated by [scripts/generate-doc-figures.php](../scripts/generate-doc-figures.php)
into `public/images/docs/`.
