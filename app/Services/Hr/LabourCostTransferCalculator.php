...
/**
 * Prices one employee's stint at another outlet: days x daily rate, plus the
 * approved overtime they claimed inside the same dates.
 *
...
class LabourCostTransferCalculator
{
    ...
    /**
     * @return array{
     *   days: float, daily_rate: float, salary_amount: float,
     *   ot_hours: float, ot_amount: float, total_amount: float,
     *   ot_claim_ids: array<int, int>, ot_claims: Collection, has_salary: bool
     * }
     */
    public function price(Employee $employee, string $start, string $end, ?float $days = null): array
    {
        $calendarDays = self::calendarDays($start, $end);
        $days = $days === null ? $calendarDays : max(0, min($days, $calendarDays));
        ...
        foreach ($claims as $claim) {
            ...
            $otHours  += $hours;
            $otAmount += $hours * $hourlyRate * $this->settings->multiplierFor((string) $claim->ot_type);
        }

        $salaryAmount = round($days * $dailyRate, 2);
     ...
     * these dates. Charging the same day twice is the mistake this whole
     * document exists to prevent, so it is refused rather than warned about.
     */
    public static function overlapping(int $employeeId, string $start, string $end, ?int $exceptTransferId = null): ?LabourCostTransferLine
    {
    ...
    {
        $blank = fn (int $id) => [
            'outlet_id' => $id, 'staff' => [], 'days' => 0.0, 'ot_hours' => 0.0,
