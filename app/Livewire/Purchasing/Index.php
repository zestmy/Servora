...
class Index extends Component
{
    ...
    public function submitPr(int $id): void
    {
        ...
        if ($requiresApproval) {
            ...
            session()->flash('success_next', 'An appointed approver will review it — it now shows under "Pending Approval".');
        } else {
            $pr->update([
                'status'      => 'approved',
                'approved_by' => Auth::id(),
            ...
            session()->flash('success_next', Auth::user()->company?->isCpuMode()
                ? 'It will be picked up in the next consolidation and turned into supplier orders.'
                : 'Convert it to a purchase order when you are ready to buy.');
        }
    }
    ...
    public function approvePr(int $id): void
    {
        ...
        session()->flash('success_next', Auth::user()->company?->isCpuMode()
            ? 'It will be picked up in the next consolidation and turned into supplier orders.'
            : 'The requester can now convert it to a purchase order.');
    }
    ...
    public function submitPo(int $id): void
    {
        ...
        if ($requiresApproval) {
            ...
            session()->flash('success_next', 'An appointed approver will review it — once approved it goes out to the supplier or purchasing team.');
        } else {
            ...
            session()->flash('success_next', 'The purchasing team will place the order with the supplier; record the goods when they arrive using "Receive" on this order.');
            }
        }
    }
    ...
    public function approvePo(int $id): void
    {
        ...
        if ($this->shouldDirectSupplierOrder()) {
            ...
            $this->autoGenerateDo($po);
            session()->flash('success', 'PO approved — a delivery order and goods received note were created automatically.');
            session()->flash('success_next', 'Find the delivery under Deliveries (DO); confirm what actually arrives under Goods Received (GRN).');
        } else {
            session()->flash('success', 'PO approved and sent to purchasing team.');
            session()->flash('success_next', 'The purchasing team will place the order with the supplier; record the goods when they arrive using "Receive" on this order.');
        }
    ...
    private function sendPoEmail(PurchaseOrder $po): void
    {
        ...
        if ($result['success']) {
            ...
            session()->flash('success_next', 'The supplier has the order — record the goods with "Receive" on this order when they arrive.');
        } else {
