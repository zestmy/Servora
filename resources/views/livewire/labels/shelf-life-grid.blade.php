<div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
            <p class="page-eyebrow">Labels / Shelf life</p>
            <h1 class="page-title mt-1">Shelf life</h1>
        </div>
        <div class="text-right">
            <p class="text-xs text-gray-600">Categories with a chilled life</p>
            <p class="text-sm font-semibold {{ $coverage['covered'] < $coverage['total'] ? 'text-warning-600' : 'text-success-600' }}">
    ...
    <div class="card p-4 mb-4">
        <div class="flex flex-wrap items-end gap-3">
            ...
            <label class="flex items-center gap-2 pb-2 text-sm text-gray-600">
                <input type="checkbox" wire:model.live="gapsOnly" class="rounded border-gray-300 text-brand-600">
            ...
            <table class="min-w-[900px] divide-y divide-gray-100 text-sm">
                <thead class="bg-gray-50 text-gray-500 uppercase text-xs tracking-wider">
                    ...
                    <tr class="bg-gray-50/60 border-t border-gray-100">
                        <th class="px-4 py-2 text-left sticky left-0 bg-gray-50 z-10 normal-case text-gray-600 font-normal">
                            Apply to this page
                        </th>
                        @foreach ($states as $state => $stateLabel)
                            <th class="px-3 py-2">
                                <div class="flex items-center gap-1">
                                    ...
                                    <button type="button" wire:click="bulkApply('{{ $state }}')"
                                            class="px-1.5 py-1 text-xs text-brand-600 hover:text-brand-800"
                ...
                <tbody class="divide-y divide-gray-50">
                                    ...
                                    <div class="flex items-center gap-1">
                                        ...
                                        @if (($cells[$row->id][$state]['value'] ?? '') !== '')
                                            <button type="button" wire:click="clearCell({{ $row->id }}, '{{ $state }}')"
                                                    class="text-gray-500 hover:text-danger-500 text-xs" title="Clear">×</button>
                                        @endif
                                    </div>
                                </td>
                            @endforeach
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ count($states) + 1 }}" class="px-4 py-10 text-center text-gray-600 text-sm">
