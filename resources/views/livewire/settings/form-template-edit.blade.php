<div>
    ...
    <div class="flex items-center gap-3 mb-6">
        <a data-back href="{{ route('settings.form-templates') }}" class="text-gray-600 hover:text-gray-900 transition flex-shrink-0">
            ...
            <div class="card p-6 space-y-4 lg:sticky lg:top-6">
                ...
                @if ($form_type === 'purchase_order')
                    <div class="pt-3 border-t border-gray-100 space-y-4">
                        ...
                        <p class="text-xs text-gray-600 -mt-2">Pre-fill these fields when this template is loaded on a new PO.</p>
        ...
        <div class="lg:col-span-2">
            <div class="card">
                ...
                <div class="px-6 py-4 border-b border-gray-100">
                    ...
                    <p class="text-xs text-gray-600 mt-0.5">These items will be pre-loaded when this template is applied to a form.</p>
                </div>
                ...
                <div class="px-6 py-4 border-b border-gray-100">
                    ...
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <div class="flex items-center gap-1.5">
                            <select id="tpl_cat_loader" class="text-xs border-gray-300 rounded-lg shadow-sm focus:border-brand-500 focus:ring-brand-500 py-1.5"
                        ...
                        <div class="mt-2 border border-gray-200 rounded-lg overflow-hidden divide-y divide-gray-100 shadow-sm">
                            ...
                            @if ($ingredientResults->isNotEmpty())
                                @if ($recipeResults->isNotEmpty())
                                    <div class="px-4 py-1.5 bg-gray-50 text-xs font-semibold text-gray-600 uppercase tracking-wider">Products</div>
                                @endif
                                @foreach ($ingredientResults as $ingredient)
                                    <button type="button" wire:click="addIngredient({{ $ingredient->id }})"
                                        ...
                                        <span class="text-xs text-gray-600 ml-4">{{ $ingredient->baseUom?->abbreviation }}
                            ...
                            @if ($recipeResults->isNotEmpty())
                                <div class="px-4 py-1.5 bg-gray-50 text-xs font-semibold text-gray-600 uppercase tracking-wider">Recipes</div>
                                @foreach ($recipeResults as $recipe)
                                    <button type="button" wire:click="addRecipe({{ $recipe->id }})"
                                        ...
                                        <span class="text-xs text-gray-600 ml-4">{{ $recipe->yieldUom?->abbreviation }}
                                            <span class="text-brand-400 ml-1">+ Add</span>
                                        </span>
                                    </button>
                                @endforeach
                    ...
                    @elseif (strlen($itemSearch) >= 2)
                        <p class="mt-2 text-sm text-gray-600 text-center py-2">No items found.</p>
                    @endif
                </div>
                                    ...
                                    <tr wire:key="ft-line-{{ $line['id'] }}" data-id="{{ $line['id'] }}" class="hover:bg-gray-50 transition group">
                                        ...
                                        <td class="px-4 py-2">
                                            <div class="flex items-center gap-2">
                                                ...
                                                @if (! empty($line['pack_info']))
                                                    <span class="text-brand-600 font-semibold text-sm">{{ $line['pack_info'] }}</span>
                                                @endif
                                        ...
                                        <td class="px-4 py-2 text-center opacity-0 group-hover:opacity-100 transition">
                                            <button type="button" wire:click="removeLine({{ $line['id'] }})"
                                                    data-confirm-delete="Remove '{{ addslashes($line['item_name']) }}' from this template?"
