    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-600 mt-0.5">Configure automated analytics reports delivered to your email</p>
            </div>
        </div>
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-gray-600">
                                ...
                                @if($subscription->delivery_day)
                                    <span class="text-gray-600 text-xs">({{ $subscription->getDeliveryDayLabel() }})</span>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <button wire:click="runNow({{ $subscription->id }})"
                                            ...
                                            title="Run now" class="text-success-500 hover:text-success-700 transition disabled:opacity-40">
                                    ...
                                    <button wire:click="delete({{ $subscription->id }})"
                                            data-confirm-delete="Delete this report subscription?"
                                            title="Delete"
                            ...
                            <div class="inline-flex items-center gap-1.5">
                                @if($log->subscription && $log->delivery_status !== 'pending')
                                    ...
                                    <button wire:click="resendReport({{ $log->id }})"
                                            ...
                                            class="px-2.5 py-1 text-xs font-medium text-brand-600 border border-brand-200 rounded-lg hover:bg-brand-50 transition disabled:opacity-50">
                ...
                <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg z-10" @click.stop>
                    ...
                    <div class="px-6 py-4 space-y-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-600 mt-1">Reports sent on the selected day each month</p>
                        </div>
                        @endif
                        ...
                        <div>
                            ...
                            <input type="time" wire:model="delivery_time"
                                   class="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-brand-500 focus:ring-brand-500" />
                            <p class="text-xs text-gray-600 mt-1">Report will be sent at this time (MYT)</p>
                        </div>
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md z-10" @click.stop>
                    ...
                    <div class="px-6 py-4 space-y-4">
                        ...
                        {{-- Result --}}
                        @if($testResult)
                            <div class="p-3 rounded-lg text-sm {{ $testSuccess ? 'bg-success-50 text-success-700' : 'bg-danger-50 text-danger-700' }}">
                                {{ $testResult }}
                            </div>
                        @endif
                    </div>
