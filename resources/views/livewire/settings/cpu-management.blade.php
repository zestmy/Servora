<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-4">
            ...
            <h2 class="page-title">Central Purchasing Units</h2>
        </div>
                    ...
                    <div class="p-5 flex items-start justify-between">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <span class="px-2 py-0.5 text-xs rounded {{ $cpu->is_active ? 'bg-success-50 text-success-600' : 'bg-danger-50 text-danger-600' }}">
                                    {{ $cpu->is_active ? 'Active' : 'Inactive' }}
                                </span>
                                <span class="px-2 py-0.5 bg-brand-50 text-brand-600 text-xs rounded">
                        ...
                        <div class="flex gap-2 ml-4">
                            ...
                            <button wire:click="delete({{ $cpu->id }})"
                                    data-confirm-delete="Are you sure you want to delete this CPU?"
        ...
        <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg z-10 p-6 mt-8 mb-8">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-600 mb-2">Branches this CPU consolidates purchasing for — their purchase requests route here automatically. Each branch is served by one CPU; selecting a branch served elsewhere moves it here.</p>
                    ...
                    <div class="max-h-40 overflow-y-auto border border-gray-200 rounded-lg p-2 space-y-1">
                        @foreach ($companyUsers as $u)
                            <label class="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-gray-50 cursor-pointer">
                                ...
                                <span class="text-xs text-gray-600">({{ $u->email }})</span>
                            </label>
                        @endforeach
                    </div>
                </div>
