<div>
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div class="flex-1">
            <p class="text-xs text-gray-600"><a href="{{ route('settings.index') }}" class="hover:underline">Settings</a> / Approvers</p>
        </div>
    </div>
    ...
    <div class="mb-8">
        ...
        <div class="card overflow-hidden">
                        ...
                        @foreach ($poApprovers as $userId => $data)
                            <tr class="hover:bg-gray-50">
                                <td class="px-5 py-3">
                                    ...
                                    <p class="text-xs text-gray-600">{{ $data['user']?->email }}</p>
                                </td>
                                <td class="px-5 py-3">
                                    @if ($data['is_all_outlets'])
                                        <span class="px-2 py-0.5 bg-brand-50 text-brand-600 text-xs rounded-full font-medium">All Outlets</span>
                                    @else
                                ...
                                <td class="px-5 py-3">
                                    @if ($data['is_all_depts'])
                                        <span class="px-2 py-0.5 bg-success-50 text-success-600 text-xs rounded-full font-medium">All Departments</span>
                                    @else
            ...
            @else
                <div class="p-8 text-center text-gray-600 text-sm">No PO approvers assigned. Click "+ Add PO Approver" to get started.</div>
            @endif
        </div>
    </div>
    ...
    <div class="mb-8">
        ...
        <div class="card overflow-hidden">
            ...
            @else
                <div class="p-8 text-center text-gray-600 text-sm">No PR approvers assigned. Click "+ Add PR Approver" to get started.</div>
            @endif
        </div>
    </div>
                ...
                <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg z-10 p-6">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            @error('selectedUserId') <p class="text-xs text-danger-500 mt-1">{{ $message }}</p> @enderror
                        </div>

                        {{-- Outlets --}}
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-2">Outlets *</label>
                            <label class="flex items-center gap-2 mb-2 px-2 py-1.5 bg-brand-50 rounded-lg cursor-pointer">
                                <input type="checkbox" wire:model.live="allOutlets" class="rounded border-gray-300 text-brand-600 focus:ring-brand-500" />
                                <span class="text-sm font-medium text-brand-700">All Outlets</span>
                            </label>
                            @if (! $allOutlets)
                            ...
                            @endif
                            @error('selectedOutletIds') <p class="text-xs text-danger-500 mt-1">{{ $message }}</p> @enderror
                        </div>

                        {{-- Departments --}}
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-2">Departments *</label>
                            <label class="flex items-center gap-2 mb-2 px-2 py-1.5 bg-success-50 rounded-lg cursor-pointer">
                                <input type="checkbox" wire:model.live="allDepartments" class="rounded border-gray-300 text-success-600 focus:ring-success-500" />
                                <span class="text-sm font-medium text-success-700">All Departments</span>
                            </label>
                            @if (! $allDepartments)
                                <div class="max-h-32 overflow-y-auto border border-gray-200 rounded-lg p-2 space-y-1">
                            ...
                            @endif
                            @error('selectedDepartmentIds') <p class="text-xs text-danger-500 mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>
