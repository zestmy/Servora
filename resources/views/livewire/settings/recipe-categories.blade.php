    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div class="flex-1">
            ...
            <p class="text-xs mt-1 flex items-center gap-1.5">
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold {{ $isKitchen ? 'bg-orange-100 text-orange-700' : 'bg-brand-100 text-brand-700' }}">
                    {{ $scopeLabel }}
                </span>
                ...
                @foreach ($categories as $cat)
                    <div data-id="{{ $cat->id }}" class="border-b border-gray-100 last:border-b-0">
                        ...
                        <div class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition group">
                            <div class="parent-drag cursor-grab active:cursor-grabbing text-gray-500 hover:text-gray-900 flex-shrink-0">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if ($cat->children->count())
                                    <span class="text-xs text-gray-600 ml-1">({{ $cat->children->count() }} sub)</span>
                                @endif
                            </div>
                            <span class="text-xs text-gray-600 tabular-nums w-12 text-center">{{ $recipeCounts[$cat->name] ?? 0 }}</span>
                            @if ($cat->is_active)
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-success-100 text-success-700">Active</span>
                            @else
                            ...
                            <div class="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition flex-shrink-0">
                                <button wire:click="openCreate({{ $cat->id }})" title="Add Sub" class="text-success-500 hover:text-success-700 p-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg>
                                </button>
                                <button wire:click="openEdit({{ $cat->id }})" title="Edit" class="text-brand-500 hover:text-brand-700 p-1">
                                ...
                                <button wire:click="toggleActive({{ $cat->id }})" title="{{ $cat->is_active ? 'Deactivate' : 'Activate' }}"
                                        class="{{ $cat->is_active ? 'text-success-500 hover:text-success-700' : 'text-gray-600 hover:text-gray-900' }} p-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                </button>
                                <button wire:click="delete({{ $cat->id }})" data-confirm-delete="Delete '{{ $cat->name }}'? {{ $isKitchen ? 'Production recipes' : 'Recipes' }} using this category will keep their category label." title="Delete"
                            ...
                            <div x-data x-init="new Sortable($el, {
                                ...
                                @foreach ($cat->children as $sub)
                                    <div data-id="{{ $sub->id }}" class="flex items-center gap-3 px-4 py-2.5 pl-10 hover:bg-gray-100/50 transition group border-b border-gray-50 last:border-b-0">
                                        <div class="sub-drag cursor-grab active:cursor-grabbing text-gray-500 hover:text-gray-900 flex-shrink-0">
                                        ...
                                        <span class="text-xs text-gray-600 tabular-nums w-12 text-center">{{ $recipeCounts[$sub->name] ?? 0 }}</span>
                                        @if ($sub->is_active)
                                            <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-success-100 text-success-700">Active</span>
                                        @else
                                            <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-gray-100 text-gray-500">Inactive</span>
                                        @endif
                                        <div class="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition flex-shrink-0">
                                            <button wire:click="openEdit({{ $sub->id }})" title="Edit" class="text-brand-500 hover:text-brand-700 p-1">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                            </button>
                                            <button wire:click="toggleActive({{ $sub->id }})" title="{{ $sub->is_active ? 'Deactivate' : 'Activate' }}"
                                                    class="{{ $sub->is_active ? 'text-success-500 hover:text-success-700' : 'text-gray-600 hover:text-gray-900' }} p-1">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                            </button>
                                            <button wire:click="delete({{ $sub->id }})" data-confirm-delete="Delete sub-category '{{ $sub->name }}'?" title="Delete"
                                                    class="text-danger-400 hover:text-danger-600 p-1">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                            </button>
                                        </div>
