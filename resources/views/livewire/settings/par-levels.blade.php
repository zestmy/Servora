<div>
    ...
    @if (session()->has('error'))
        <div wire:key="err-{{ microtime(true) }}" class="mb-4 px-4 py-3 bg-danger-50 border border-danger-200 text-danger-700 text-sm rounded-lg">
            {{ session('error') }}
        </div>
    @endif
    ...
    <div class="flex items-center gap-3 mb-6">
        <a data-back href="{{ route('settings.index') }}" class="text-gray-600 hover:text-gray-900 transition flex-shrink-0">
        ...
        <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-gray-600">Set par levels per ingredient per outlet. Used for auto-calculating order quantities.</p>
        </div>
        <span class="px-2.5 py-1 rounded-full bg-brand-50 text-brand-600 font-medium text-xs whitespace-nowrap">
    ...
    <div x-data="{ open: localStorage.getItem('parTipDismissed') !== '1' }" class="mb-4">
        <div x-show="open" x-collapse>
            <div class="relative bg-brand-50/60 border border-brand-100 rounded-xl p-4 pr-10 text-sm text-gray-600">
                <button type="button" @click="open = false; localStorage.setItem('parTipDismissed', '1')"
                        title="Dismiss" class="absolute top-3 right-3 text-gray-600 hover:text-gray-900">
                ...
                <ul class="space-y-1 list-disc list-inside marker:text-brand-300">
                    ...
                    <li><span class="font-medium text-gray-700">Need a starting point?</span> Tap <span class="text-brand-600">≈ N ↑</span> on a row to use a value suggested from the last 3 months of purchases, or use <span class="font-medium text-gray-700">Fill blanks with suggested</span>.</li>
                    ...
                    <li>The <span class="font-medium text-gray-700">On hand</span> column shows current stock; a <span class="text-danger-600 font-medium">red value</span> means it's below par.</li>
                </ul>
            </div>
        </div>
        <button type="button" x-show="!open" @click="open = true; localStorage.removeItem('parTipDismissed')"
                class="text-xs text-brand-500 hover:text-brand-700 flex items-center gap-1">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-3">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div>
                ...
                <select wire:model.live="outletId" class="w-full rounded-lg border-gray-300 text-sm focus:ring-brand-500 focus:border-brand-500">
            ...
            <div>
                ...
                <select wire:model.live="categoryFilter" class="w-full rounded-lg border-gray-300 text-sm focus:ring-brand-500 focus:border-brand-500">
            ...
            <div>
                ...
                <select wire:model.live="statusFilter" class="w-full rounded-lg border-gray-300 text-sm focus:ring-brand-500 focus:border-brand-500">
        ...
        <div class="flex flex-wrap items-end gap-x-6 gap-y-3">
            <div>
                ...
                <div class="flex items-center gap-2">
                    <input type="number" step="0.01" min="0" wire:model.live.debounce.400ms="bulkValue" placeholder="Qty"
                           class="w-24 rounded-lg border-gray-300 text-sm text-right focus:ring-brand-500 focus:border-brand-500" />
                    <label class="inline-flex items-center gap-1.5 text-xs text-gray-500">
                        <input type="checkbox" wire:model="bulkAllOutlets" class="rounded border-gray-300 text-brand-600 focus:ring-brand-500" />
                        All outlets
                    </label>
            ...
            <div>
                ...
                <button wire:click="fillBlanksWithSuggested"
                        ...
                        class="px-3 py-2 border border-brand-200 text-brand-600 text-sm font-medium rounded-lg hover:bg-brand-50 transition">
            ...
            <div>
                ...
                <div class="flex items-center gap-2">
                    <select wire:model.live="copyFromOutletId" class="rounded-lg border-gray-300 text-sm focus:ring-brand-500 focus:border-brand-500">
                ...
                <tbody>
                    @forelse ($ingredients as $ingredient)
                        ...
                        <tr wire:key="par-row-{{ $ingredient->id }}" class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-2.5 text-right tabular-nums {{ $belowPar ? 'text-danger-600 font-semibold' : 'text-gray-500' }}">
                                @if ($oh === null)
                                    —
                                @else
                                    {{ rtrim(rtrim(number_format($oh, 2), '0'), '.') }}
                                    @if ($belowPar)
                                        <span title="Below par level" class="inline-block w-1.5 h-1.5 rounded-full bg-danger-500 ml-0.5 align-middle"></span>
                                    @endif
                                @endif
                            </td>
