<div>
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <p class="text-xs text-gray-600"><a href="{{ route('settings.index') }}" class="hover:underline">Settings</a> / API Keys</p>
        </div>
    </div>
        ...
        <div class="card p-6">
            ...
            <div x-data="{ visible: false }" class="mb-4">
                ...
                <div class="mt-1 flex items-center gap-2">
                    <div class="flex-1 relative">
                        <input id="openrouter_key"
                               ...
                               placeholder="sk-or-…"
                               class="block w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-brand-500 focus:ring-brand-500 pr-10"
                               autocomplete="off" />
                ...
                <p class="mt-1.5 text-xs text-gray-600">
                    Get your key from <a href="https://openrouter.ai/keys" target="_blank" class="text-brand-600 hover:underline">openrouter.ai/keys</a>.
                </p>
            </div>
            ...
            <div class="mb-4" x-data="{ custom: @js(!in_array($openrouter_model, ['', 'anthropic/claude-sonnet-4', 'anthropic/claude-haiku-3.5', 'google/gemini-2.5-flash-preview', 'google/gemini-2.5-pro-preview', 'openai/gpt-4.1', 'openai/gpt-4.1-mini', 'meta-llama/llama-4-maverick', 'deepseek/deepseek-r1'])) }">
                ...
                <div x-show="custom" class="mt-1 flex items-center gap-2">
                    ...
                    <button type="button" @click="custom = false; $wire.set('openrouter_model', '')"
                            class="px-2 py-2 text-gray-600 hover:text-gray-900 transition" title="Back to list">
            ...
            @if ($openrouter_key)
                <div class="mt-3 flex items-center gap-1.5 text-xs text-success-600">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    API key is configured
                </div>
            @else
                <div class="mt-3 flex items-center gap-1.5 text-xs text-warning-600">
        ...
        <div class="card p-6">
            <div class="flex items-start gap-4 mb-5">
                <div class="w-10 h-10 bg-success-100 rounded-lg flex items-center justify-center flex-shrink-0">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-success-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div class="mb-4">
                ...
                <p class="mt-1 text-xs text-gray-600">The "From" email address. Must be a verified sender in EngineMailer.</p>
            </div>
            ...
            @if ($testResult)
                <div class="mt-3 px-4 py-3 rounded-lg text-sm {{ $testSuccess ? 'bg-success-50 border border-success-200 text-success-700' : 'bg-danger-50 border border-danger-200 text-danger-700' }}">
                    {{ $testResult }}
                </div>
            @endif
