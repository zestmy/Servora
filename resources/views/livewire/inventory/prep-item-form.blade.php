<div>
                                ...
                                <td class="px-4 py-2">
                                    <div class="relative">
                                        <input type="number" step="0.1" min="0" max="100"
                                               wire:model.blur="lines.{{ $idx }}.waste_percentage" wire:loading.attr="disabled" wire:target="reorderLines,reorderPackagingLines"
    ...
    <div class="mt-4 card p-6">
        ...
        @if (count($existingPresentationImages))
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 mb-4">
                @foreach ($existingPresentationImages as $img)
                    <div class="relative group rounded-lg overflow-hidden border border-gray-200 bg-gray-50" wire:key="pres-img-{{ $img['id'] }}">
        ...
        <div>
            ...
            <x-input-error :messages="$errors->get('newPresentationImages.*')" class="mt-1" />
        </div>
    ...
    <div class="mt-4 card" x-data="{ sopOpen: {{ count($steps) || $video_url ? 'true' : 'false' }} }">
        ...
        <div x-show="sopOpen" x-cloak class="px-6 pb-6 space-y-4 border-t border-gray-100">
            ...
            <x-audit-timeline :type="\App\Models\Recipe::class" :id="$recipeId" title="Prep Item Activity" />
