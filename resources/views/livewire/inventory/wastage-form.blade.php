<div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2">
            <div class="card p-6 space-y-4">
                ...
                @if (! $recordId)
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-600">
                            {{ $method === 'summary' ? 'Enter the total wastage value directly — quick entry when itemising isn\'t worth it.' : 'List each item wasted individually.' }}
                        </p>
                    </div>
                ...
                @if ($method === 'summary')
                    <div>
                        <x-input-label for="w_amount" value="Total Wastage Value (RM) *" />
                        <x-text-input id="w_amount" wire:model="summary_amount" type="number" step="0.01" min="0"
                                      class="mt-1 block w-full text-lg font-semibold" placeholder="0.00" />
                        <p class="mt-1 text-xs text-gray-600">Enter the total wastage value for this record.</p>
                ...
                <dl class="space-y-3 text-sm">
                    @if ($method === 'summary')
                        ...
                        <div class="flex justify-between border-t border-gray-100 pt-3">
                            <dt class="font-semibold text-gray-600">Total Wastage Value</dt>
                            <dd class="font-bold text-lg text-danger-600 tabular-nums">
    ...
    <div class="mt-4 card">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <div>
                ...
                <p class="text-xs text-gray-600 mt-0.5">{{ count($lines) }} item{{ count($lines) !== 1 ? 's' : '' }} — ingredients, prep items, or finished recipes</p>
            </div>
        ...
        <div class="px-6 py-4 border-b border-gray-100">
            ...
            @if ($ingredientResults->isNotEmpty() || $recipeResults->isNotEmpty())
                <div class="mt-2 border border-gray-200 rounded-lg overflow-hidden divide-y divide-gray-100 shadow-sm">
                    ...
                    @if ($ingredientResults->isNotEmpty())
                        <div class="px-4 py-1.5 bg-gray-50 text-xs font-semibold text-gray-600 uppercase tracking-wider">
                            Products
                        </div>
                        ...
                        @foreach ($recipeResults as $recipe)
                            <button type="button" wire:click="addRecipe({{ $recipe->id }})"
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @if ($recipe->category)
                                        <span class="text-xs text-gray-600">· {{ $recipe->category }}</span>
                                    @endif
                                </div>
                                <div class="text-right text-xs flex-shrink-0 ml-4">
                                    <span class="text-gray-600">
            ...
            @elseif (strlen($itemSearch) >= 2)
                <p class="mt-2 text-sm text-gray-600 text-center py-2">No ingredients or recipes found.</p>
            @endif
            ...
            <div class="overflow-x-auto">
                <table class="table-surface min-w-full">
                    ...
                    <tfoot class="bg-gray-50 border-t-2 border-gray-200 text-sm font-semibold">
                        <tr>
                            <td colspan="6" class="px-4 py-3 text-right text-gray-600">Total</td>
    ...
    <x-audit-timeline :type="\App\Models\WastageRecord::class" :id="$recordId" title="Wastage Activity" class="mt-4" />
</div>
