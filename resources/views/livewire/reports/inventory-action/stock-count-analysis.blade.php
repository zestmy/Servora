<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="page-title">Stock Count Analysis</h2>
            <p class="text-xs text-gray-600 mt-0.5">Variance analysis between expected and counted quantities</p>
        </div>
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse ($lines as $line)
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 text-right tabular-nums font-medium {{ $isNeg ? 'text-danger-600' : ($isPos ? 'text-success-600' : 'text-gray-500') }}">
                            {{ $isPos ? '+' : '' }}{{ number_format($line->variance_quantity, 2) }}
                        </td>
                        <td class="px-4 py-3 text-right tabular-nums {{ $isNeg ? 'text-danger-600' : ($isPos ? 'text-success-600' : 'text-gray-500') }}">
