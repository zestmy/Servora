<div>
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <h2 class="page-title">Invoice Summary</h2>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
        ...
        <div class="card p-4">
            ...
            <p class="text-xl font-bold text-success-600 mt-1">{{ number_format((float) $totalPaid, 2) }}</p>
        </div>
        <div class="card p-4">
            <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">Overdue</p>
            <p class="text-xl font-bold text-danger-600 mt-1">{{ number_format((float) $totalOverdue, 2) }}</p>
        </div>
    </div>
                    ...
                    @forelse ($invoices as $inv)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-gray-600">
                                ...
                                @else
                                    <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-brand-100 text-brand-700">Supplier</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-gray-600">
                            ...
                            <td class="px-4 py-3">
                                <span @class([
                                    ...
                                    'bg-success-100 text-success-700'   => $inv->status === 'paid',
                                    'bg-danger-100 text-danger-700'       => in_array($inv->status, ['overdue', 'cancelled']),
                                ])>{{ ucfirst($inv->status) }}</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-8 text-center text-gray-600">No invoices found for this period.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
