<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse ($plans as $plan)
            <div class="card p-5 {{ !$plan->is_active ? 'opacity-60' : '' }}">
                <div class="flex items-start justify-between mb-3">
                    ...
                    <div class="flex items-center gap-1.5">
                        @if ($plan->is_active)
                            <span class="px-2 py-0.5 bg-success-100 text-success-700 text-xs font-medium rounded-full">Active</span>
                        @else
                            <span class="px-2 py-0.5 bg-gray-100 text-gray-500 text-xs font-medium rounded-full">Inactive</span>
                        @endif
                        @unless ($plan->is_public)
                            <span class="px-2 py-0.5 bg-warning-100 text-warning-700 text-xs font-medium rounded-full" title="Not shown on the public pricing page — assignable by admins and reachable via direct links">Hidden</span>
                        @endunless
                    </div>
                </div>
                ...
                <p class="text-xs text-gray-600 -mt-3 mb-4">
                    ...
                    @if ($plan->yearlyDiscount() > 0)
                        <span class="text-success-600 font-medium">(save {{ $plan->yearlyDiscount() }}%)</span>
                    @endif
                </p>
                ...
                <div class="flex flex-wrap gap-1.5 mb-4">
                    @foreach ($plan->feature_flags ?? [] as $flag => $enabled)
                        <span class="px-2 py-0.5 rounded-full text-[10px] font-medium
                            {{ $enabled ? 'bg-brand-50 text-brand-700' : 'bg-gray-50 text-gray-600 line-through' }}">
                ...
                <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                    <span class="text-xs text-gray-600">{{ $plan->subscriptions_count }} {{ Str::plural('subscriber', $plan->subscriptions_count) }}</span>
                    <div class="flex items-center gap-2">
                        <button wire:click="toggleActive({{ $plan->id }})"
                                title="{{ $plan->is_active ? 'Deactivate' : 'Activate' }}"
                                class="{{ $plan->is_active ? 'text-success-500 hover:text-success-700' : 'text-gray-600 hover:text-gray-900' }} transition">
        ...
        @empty
            <div class="col-span-full card p-12 text-center text-gray-600">
