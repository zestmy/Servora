    ...
    <div class="card overflow-hidden">
        <table class="table-surface min-w-full">
            ...
            <tbody>
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 text-center">
                            @if ($program->is_recurring)
                                <span class="text-success-600 text-xs font-medium">Yes</span>
                            @else
                                <span class="text-gray-600 text-xs">First only</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-center text-gray-600">{{ $program->max_payouts ?? '∞' }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium {{ $program->is_active ? 'bg-success-100 text-success-700' : 'bg-gray-100 text-gray-500' }}">
                                {{ $program->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button wire:click="openEdit({{ $program->id }})" class="text-brand-500 hover:text-brand-700 transition">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </button>
                                <button wire:click="delete({{ $program->id }})" data-confirm-delete="Delete this referral program?" class="text-danger-400 hover:text-danger-600 transition">
                ...
                @empty
                    <tr><td colspan="6" class="px-4 py-12 text-center text-gray-600">No referral programs yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
            ...
            <form wire:submit="save">
                <div class="px-6 py-5 space-y-4">
                    <div>
                        ...
                        <select id="rp_plan" wire:model="plan_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-brand-500 focus:ring-brand-500">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="rp_type" wire:model="commission_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-brand-500 focus:ring-brand-500">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="flex items-end gap-4 pb-1">
                            <label class="inline-flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" wire:model="is_recurring" class="rounded border-gray-300 text-brand-600 shadow-sm focus:ring-brand-500" />
