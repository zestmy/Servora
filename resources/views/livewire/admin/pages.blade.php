<div>
                    ...
                    @forelse ($pages as $page)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-4 py-3">
                                ...
                                @else
                                    <p class="text-xs text-gray-600">{{ Str::limit(strip_tags($page->content), 60) }}</p>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3 text-center">
                                @if ($page->menu_placement)
                                    <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-brand-50 text-brand-700">{{ ucfirst($page->menu_placement) }}</span>
                                @else
                            ...
                            <td class="px-4 py-3 text-center">
                                <button wire:click="togglePublish({{ $page->id }})"
                                        class="px-2 py-0.5 rounded-full text-xs font-medium {{ $page->is_published ? 'bg-success-100 text-success-700' : 'bg-gray-100 text-gray-500' }}">
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    @if ($page->is_published)
                                        <a href="{{ $page->url() }}" target="_blank" title="View" class="text-gray-600 hover:text-gray-900 transition">
                                    ...
                                    @endif
                                    <button wire:click="edit({{ $page->id }})" title="Edit" class="text-brand-500 hover:text-brand-700 transition">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </button>
                                    <button wire:click="delete({{ $page->id }})" data-confirm-delete="Delete '{{ $page->title }}'?" title="Delete" class="text-danger-400 hover:text-danger-600 transition">
    ...
    @else
        {{-- Editor View --}}
        <div class="flex items-center gap-3 mb-4">
            <button wire:click="cancel" class="text-gray-600 hover:text-gray-900 transition">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
                </svg>
            </button>
            <div>
                <p class="text-xs text-gray-600"><a href="#" wire:click.prevent="cancel" class="hover:underline">Pages</a> / {{ $editingId ? 'Edit' : 'Create' }}</p>
            </div>
        </div>
        ...
        <form wire:submit="save" class="max-w-4xl space-y-6">
            ...
            <div class="card p-6">
                ...
                <div class="mb-4">
                    ...
                    <p class="text-xs text-gray-600 mt-1">If set, this menu item links to an external URL instead of showing page content.</p>
                </div>
            ...
            <div class="card p-6">
                ...
                <p class="text-xs text-gray-600 mb-3">Supports HTML. Use headings, paragraphs, lists, and links.</p>
                <div x-data="{
                    tab: 'write',
                    ...
                    <div class="flex gap-2 mb-3 border-b border-gray-200">
                        <button type="button" @click="tab = 'write'"
                                :class="tab === 'write' ? 'border-brand-600 text-brand-600' : 'border-transparent text-gray-500'"
                                class="pb-2 text-sm font-medium border-b-2 transition">Write</button>
                        <button type="button" @click="tab = 'preview'"
                                :class="tab === 'preview' ? 'border-brand-600 text-brand-600' : 'border-transparent text-gray-500'"
                    ...
                    <textarea x-show="tab === 'write'"
                              ...
                              rows="20"
                              class="block w-full rounded-md border-gray-300 shadow-sm text-sm font-mono focus:border-brand-500 focus:ring-brand-500"
...
<p>Your privacy is important to us...</p>"></textarea>
                    ...
                    <div x-show="tab === 'preview'" x-cloak
                         ...
                         x-html="content || '<p class=\'text-gray-600\'>Nothing to preview yet.</p>'">
                    </div>
                </div>
