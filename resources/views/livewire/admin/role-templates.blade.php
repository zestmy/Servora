<div>
    ...
    <div class="mb-6 px-4 py-3 bg-warning-50 border border-warning-200 text-warning-800 text-xs rounded-lg">
    ...
    <div class="space-y-4">
        @foreach ($roles as $role)
            ...
            @if ($e)
                <div wire:key="role-{{ $role->id }}" class="card p-5">
                    <div class="flex flex-wrap items-start justify-between gap-3 mb-4">
                        <div class="flex-1 min-w-[260px]">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <span class="text-[10px] uppercase tracking-wider text-gray-600" title="Stable internal key used by the system — not editable">key: {{ $role->name }}</span>
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium bg-brand-100 text-brand-700">
                                    {{ $userCounts[$role->id] ?? 0 }} {{ \Illuminate\Support\Str::plural('user', $userCounts[$role->id] ?? 0) }}
                                </span>
                            </div>
                            @error("edit.{$role->id}.display_name") <p class="text-xs text-danger-500 mt-1">{{ $message }}</p> @enderror
