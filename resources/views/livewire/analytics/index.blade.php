<div>
    ...
    @unless ($hasApiKey)
        <div class="mb-6 px-4 py-3 bg-warning-50 border border-warning-200 text-warning-700 text-sm rounded-lg flex items-center gap-2">
    ...
    {{-- Tabs. Scroller wrapper same as the reports strip: `w-fit` is
         shrink-to-fit, so two tabs pass today but the third would push the
         page sideways — the exact bug /reports/cost-summary had. --}}
    <div class="mb-6 overflow-x-auto">
        <div class="flex gap-1 bg-gray-100 rounded-lg p-1 w-fit">
            <button wire:click="switchTab('generate')"
                    class="px-4 py-2 text-sm font-medium rounded-md transition whitespace-nowrap {{ $activeTab === 'generate' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                Generate Analysis
            </button>
            <button wire:click="switchTab('saved')"
                    class="px-4 py-2 text-sm font-medium rounded-md transition whitespace-nowrap {{ $activeTab === 'saved' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                Saved Reports
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="card p-4">
                        ...
                        @foreach ($analysisTypes as $value => $label)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" wire:model.live="analysisType" value="{{ $value }}"
                                       class="text-brand-600 focus:ring-brand-500 border-gray-300" />
                    ...
                    @if ($analysisType === 'weekly_review')
                        <div class="mt-3">
                            ...
                            <p class="mt-1 text-xs text-gray-600">Select the Monday of the week to review.</p>
                        </div>
                    @elseif ($analysisType === 'custom')
                        <textarea wire:model="customQuestion" rows="3" placeholder="Type your question..."
                                  class="mt-3 block w-full rounded-md border-gray-300 text-sm focus:border-brand-500 focus:ring-brand-500"></textarea>
                    @endif
                </div>
            ...
            <div class="lg:col-span-2">
                ...
                <div wire:loading wire:target="runAnalysis"
                     class="card p-12 flex flex-col items-center justify-center">
                    ...
                    <p class="text-xs text-gray-600 mt-1">This may take up to 60 seconds</p>
                </div>

                <div wire:loading.remove wire:target="runAnalysis">
                    @if ($error)
                        <div class="bg-danger-50 border border-danger-200 rounded-xl p-4 text-sm text-danger-700">
                    ...
                    @elseif ($isMultiOutlet && count($outletResults) > 0)
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="bg-white rounded-xl shadow-sm border border-brand-200 overflow-hidden">
                                <div class="px-6 py-4 bg-brand-50/50 flex items-center justify-between flex-wrap gap-2">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-600">{{ $respondedAt }}{{ $cached ? ' (cached)' : '' }} &middot; {{ count($outletResults) }} outlets analyzed</p>
                                        </div>
                                    </div>
                                ...
                                <div class="space-y-4">
                                    @if ($outletInsights)
                                        ...
                                        @if (!empty($outletInsights['headline']))
                                            <div class="bg-gradient-to-r from-brand-500 to-purple-600 rounded-xl p-6 text-white">
                                                <div class="flex items-start justify-between gap-4">
                                                    <div>
                                                        <div class="text-xs font-medium text-brand-200 mb-1">{{ $currentOutlet['outlet_name'] }}</div>
                                                        <p class="text-lg font-semibold">{{ $outletInsights['headline'] }}</p>
                                                        @if (!empty($outletInsights['performance_score']))
                                                            @php
                                                                $scoreColors = [
                                                                    'excellent' => 'bg-success-400',
                                                                    'good' => 'bg-emerald-400',
                                                ...
                                                @foreach ($outletInsights['key_metrics'] as $metric)
                                                    <div class="card p-4">
                                                        ...
                                                        @if (!empty($metric['trend']))
                                                            <div class="mt-1 text-xs {{ $metric['trend'] === 'up' ? 'text-success-600' : ($metric['trend'] === 'down' ? 'text-danger-600' : 'text-gray-600') }}">
                                                ...
                                                <div class="bg-success-50 rounded-xl border border-success-100 p-4">
                                                    ...
                                                    <ul class="space-y-2">
                                                        @foreach ($outletInsights['highlights'] as $highlight)
                                                            <li class="text-sm text-success-800 flex items-start gap-2">
                                                ...
                                                <div class="bg-danger-50 rounded-xl border border-danger-100 p-4">
                                                    ...
                                                    <ul class="space-y-2">
                                                        @foreach ($outletInsights['concerns'] as $concern)
                                                            <li class="text-sm text-danger-800 flex items-start gap-2">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="bg-white rounded-xl shadow-sm border border-brand-200 overflow-hidden">
                                <div class="px-6 py-4 bg-brand-50/50 flex items-center justify-between flex-wrap gap-2">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 bg-brand-100 rounded-lg flex items-center justify-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-brand-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                                        </div>
                                        <div>
                                            <h3 class="text-sm font-semibold text-gray-800">AI Analysis Report</h3>
                                            <p class="text-xs text-gray-600">{{ $respondedAt }}{{ $cached ? ' (cached)' : '' }}</p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-3">
                                        <div class="text-xs text-gray-600">Powered by Servora AI</div>
                                                        ...
                                                        <tr class="hover:bg-gray-50">
                                                            ...
                                                            <td class="py-2.5 px-4 text-center">
                                                                ...
                                                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium
                                                                    {{ $trend === 'up' ? 'bg-success-100 text-success-700' : ($trend === 'down' ? 'bg-danger-100 text-danger-700' : 'bg-gray-100 text-gray-600') }}">
                                                                    @if ($trend === 'up')
                    ...
                    @else
                        ...
                        <div class="card p-12 flex flex-col items-center justify-center text-center">
                            <div class="w-16 h-16 bg-brand-50 rounded-2xl flex items-center justify-center mb-4">
    ...
    @else
        ...
        <div class="space-y-4">
            @forelse ($savedReports as $report)
                <div class="card hover:border-brand-200 transition {{ $viewingLogId === $report->id ? 'ring-2 ring-brand-500' : '' }}">
                    <div class="px-5 py-4 flex items-center justify-between gap-4">
                        <div class="flex-1 min-w-0 cursor-pointer" wire:click="loadReport({{ $report->id }})">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <span class="px-2 py-0.5 bg-brand-50 text-brand-600 text-xs font-medium rounded-full">
                    ...
                    <div class="px-5 pb-4">
                        <p class="text-xs text-gray-600 line-clamp-2">{{ Str::limit(strip_tags($report->response_text), 200) }}</p>
                    </div>
                </div>
            @empty
                <div class="card p-12 flex flex-col items-center justify-center text-center">
                    ...
                    <p class="text-xs text-gray-600 max-w-sm">Generate your first AI analysis to see it saved here.</p>
                </div>
            @endforelse
