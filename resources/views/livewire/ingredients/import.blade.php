<div>
    ...
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('ingredients.index') }}" class="text-gray-600 hover:text-gray-900 transition flex-shrink-0">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-xs text-left">
                    ...
                    <tbody class="divide-y divide-gray-50 text-gray-600">
                        ...
                        <tr><td class="px-3 py-2 font-mono font-medium">base_uom</td><td class="px-3 py-2"><span class="text-danger-500 font-semibold">Yes</span></td><td class="px-3 py-2">Purchasing unit (abbreviation or name)</td><td class="px-3 py-2 font-mono">kg</td></tr>
                        <tr><td class="px-3 py-2 font-mono font-medium">recipe_uom</td><td class="px-3 py-2 text-gray-600">No</td><td class="px-3 py-2">Recipe unit (defaults to base UOM)</td><td class="px-3 py-2 font-mono">g</td></tr>
                        <tr><td class="px-3 py-2 font-mono font-medium">purchase_price</td><td class="px-3 py-2 text-gray-600">No</td><td class="px-3 py-2">Price per pack (default: 0)</td><td class="px-3 py-2 font-mono">42.69</td></tr>
                        <tr><td class="px-3 py-2 font-mono font-medium">pack_size</td><td class="px-3 py-2 text-gray-600">No</td><td class="px-3 py-2">Pack size in base UOM (default: 1)</td><td class="px-3 py-2 font-mono">1.2</td></tr>
                        <tr><td class="px-3 py-2 font-mono font-medium">yield_percent</td><td class="px-3 py-2 text-gray-600">No</td><td class="px-3 py-2">Yield % from 0.01–100 (default: 100)</td><td class="px-3 py-2 font-mono">80</td></tr>
                        ...
                        <tr><td class="px-3 py-2 font-mono font-medium">supplier</td><td class="px-3 py-2 text-gray-600">No</td><td class="px-3 py-2">Default supplier name (auto-created if not found)</td><td class="px-3 py-2 font-mono">ABC Foods Sdn Bhd</td></tr>
                        <tr><td class="px-3 py-2 font-mono font-medium">type</td><td class="px-3 py-2 text-gray-600">No</td><td class="px-3 py-2">"ingredient" or "prep" (auto-detected by AI if omitted)</td><td class="px-3 py-2 font-mono">prep</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    @elseif ($step === 'mapping')
        ...
        @elseif ($aiError)
            <div class="mb-4 px-4 py-3 bg-warning-50 border border-warning-200 text-warning-800 text-sm rounded-xl">
                {{ $aiError }}
            </div>
        @else
        ...
        <div class="card p-5 mb-4">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-xs text-gray-600">{{ count($fileHeaders) }} columns detected · {{ count($fileDataRows) }} data rows</span>
            </div>

            <p class="text-xs text-gray-500 mb-4">Map each system field to a column from your file. Fields marked with <span class="text-danger-500 font-semibold">*</span> are required.</p>
                    ...
                    <div class="flex items-center gap-3 p-3 rounded-lg border {{ ($info['required'] && empty($columnMapping[$sysField])) ? 'border-danger-200 bg-danger-50' : 'border-gray-100 bg-gray-50' }}">
                        ...
                        <div class="flex-1">
                            <select wire:model.live="columnMapping.{{ $sysField }}"
                                    class="w-full text-xs border-gray-200 rounded-lg shadow-sm focus:ring-brand-500 focus:border-brand-500 {{ !empty($columnMapping[$sysField]) ? 'text-gray-800 bg-white' : 'text-gray-600 bg-white' }}">
    ...
    @elseif ($step === 'preview')
        ...
        <div class="mb-4 flex items-center gap-4 flex-wrap">
            ...
            <div class="flex items-center gap-2 px-4 py-2 bg-success-50 rounded-lg border border-success-200 text-sm">
                <span class="text-success-600">Products:</span>
                <span class="font-semibold text-success-700">{{ $ingredientCount }}</span>
            </div>
            @if ($prepCount > 0)
            ...
            @endif
            @if ($uomFixCount > 0)
                <div class="flex items-center gap-2 px-4 py-2 bg-warning-50 rounded-lg border border-warning-200 text-sm">
                    <span class="text-warning-600">UOM to fix:</span>
                    <span class="font-semibold text-warning-700">{{ $uomFixCount }}</span>
                </div>
            @endif
            @if ($totalRows - $validRows - $uomFixCount > 0)
                <div class="flex items-center gap-2 px-4 py-2 bg-danger-50 rounded-lg border border-danger-200 text-sm">
                    <span class="text-danger-600">Errors (skipped):</span>
                    <span class="font-semibold text-danger-700">{{ $totalRows - $validRows - $uomFixCount }}</span>
                </div>
            @endif
        </div>
        ...
        <div class="card overflow-hidden mb-4">
            <div class="px-5 py-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                ...
                <div class="flex items-center gap-4 text-xs text-gray-600">
                    <span>Supplier: <span class="text-success-700">matched</span> · <span class="text-blue-700">new</span></span>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach ($rows as $idx => $row)
                            ...
                            <tr class="{{ $row['skip'] && !$hasOnlyUomIssue ? 'bg-danger-50' : ($hasOnlyUomIssue ? 'bg-warning-50' : ($row['is_prep'] ? 'bg-orange-50/50' : 'hover:bg-gray-50')) }} transition">
                                ...
                                <td class="px-3 py-2 font-medium {{ !empty($row['errors']) ? 'text-danger-700' : 'text-gray-800' }}">
                                    {{ $row['name'] ?: '—' }}
                                </td>
                                ...
                                <td class="px-3 py-2">
                                    @if (!empty($row['base_uom_needsfix']))
                                        <div>
                                            <span class="text-warning-600 text-[10px] font-medium block mb-1">{{ $row['base_uom_label'] ? '"'.$row['base_uom_label'].'" not found' : 'Not set' }}</span>
                                            <select wire:change="fixBaseUom({{ $idx }}, $event.target.value)"
                                                    class="w-full text-[11px] border-warning-300 bg-warning-50 rounded px-1.5 py-1 focus:ring-brand-500 focus:border-brand-500">
                                ...
                                <td class="px-3 py-2">
                                    @if (!empty($row['recipe_uom_needsfix']))
                                        <div>
                                            <span class="text-warning-600 text-[10px] font-medium block mb-1">{{ $row['recipe_uom_label'] ? '"'.$row['recipe_uom_label'].'" not found' : 'Not set' }}</span>
                                ...
                                <td class="px-3 py-2 text-right tabular-nums">
                                    ...
                                    @else
                                        <span class="text-warning-500 text-[10px]">TBD</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-3 py-2 text-gray-600">
                                    ...
                                    @elseif ($row['supplier_label'])
                                        @if ($row['supplier_id'])
                                            <span class="text-success-700">{{ $row['supplier_label'] }}</span>
                                ...
                                <td class="px-3 py-2">
                                    ...
                                    @elseif ($hasOnlyUomIssue)
                                        <span class="text-warning-600">&#9998; Select UOM to include</span>
                                    @elseif ($row['is_prep'])
