<div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
            <p class="text-xs text-gray-600">HR / Documents</p>
    ...
    @if (!$isConfigured)
        <div class="bg-warning-50 border border-warning-200 rounded-xl p-6 mb-6">
            <div class="flex items-start gap-3">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-warning-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
                <div>
                    <h3 class="font-semibold text-warning-800">Google Drive API Not Configured</h3>
                    <p class="text-sm text-warning-700 mt-1">
                        To enable the file browser, please set up Google Drive API credentials.
                    </p>
                    @if ($canManage)
                        <div class="mt-3 p-3 bg-white rounded-lg border border-warning-100">
                    ...
                    <div class="divide-y divide-gray-50 max-h-[60vh] overflow-y-auto">
                        @foreach ($folders as $folder)
                            <button wire:click="setActiveFolder({{ $folder->id }})"
                                    class="w-full px-4 py-3 text-left transition hover:bg-gray-50 {{ $activeFolder === $folder->id ? 'bg-brand-50 border-l-2 border-brand-600' : '' }}">
                                <div class="font-medium text-sm {{ $activeFolder === $folder->id ? 'text-brand-700' : 'text-gray-700' }}">
                                    {{ $folder->name }}
                                </div>
                                @if ($folder->description)
                                    <div class="text-xs text-gray-600 mt-0.5 line-clamp-1">{{ $folder->description }}</div>
                                @endif
                            </button>
                        @endforeach
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div class="px-4 py-3 border-b border-gray-100 flex flex-wrap items-center gap-3">
                            ...
                            <div class="flex-1 flex items-center gap-1 text-sm overflow-x-auto">
                                <button wire:click="navigateToRoot" class="text-brand-600 hover:underline whitespace-nowrap">
                                    {{ $currentFolder->name }}
                                </button>
                                @foreach ($breadcrumbs as $index => $crumb)
                                    <svg class="h-4 w-4 text-gray-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                                    <button wire:click="navigateToBreadcrumb({{ $index }})" class="text-brand-600 hover:underline whitespace-nowrap">
                                        {{ $crumb['name'] }}
                                    </button>
                                @endforeach
                            </div>
                            ...
                            {{-- View Toggle --}}
                            <button wire:click="toggleViewMode" class="p-2 text-gray-600 hover:text-gray-900 rounded-lg hover:bg-gray-100" title="Toggle view">
                                @if ($viewMode === 'grid')
                        ...
                        <div class="px-4 py-2 border-b border-gray-100 bg-gray-50">
                            <div class="relative">
                                <input type="text"
                                       ...
                                       class="w-full pl-9 pr-8 py-2 text-sm rounded-lg border-gray-200 focus:border-brand-300 focus:ring focus:ring-brand-200 focus:ring-opacity-50">
                                <svg class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                                </svg>
                                @if ($searchQuery)
                                    <button wire:click="clearSearch" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 hover:text-gray-900">
                                    ...
                                    @foreach ($files as $file)
                                        @if ($file['isFolder'])
                                            <button wire:click="navigateToFolder('{{ $file['id'] }}')"
                                                ...
                                                <div class="w-12 h-12 flex items-center justify-center text-warning-500 mb-2">
                                                ...
                                                <span class="text-xs font-medium text-gray-700 line-clamp-2 group-hover:text-brand-600">{{ $file['name'] }}</span>
                                            </button>
                                        @else
                                            <button wire:click="openPreview('{{ $file['id'] }}')"
                                                ...
                                                @else
                                                    <div class="w-12 h-12 flex items-center justify-center mb-2 {{ $file['icon'] === 'pdf' ? 'text-danger-500' : ($file['icon'] === 'doc' ? 'text-blue-500' : ($file['icon'] === 'sheet' ? 'text-success-500' : ($file['icon'] === 'slides' ? 'text-yellow-500' : ($file['icon'] === 'image' ? 'text-purple-500' : ($file['icon'] === 'video' ? 'text-pink-500' : 'text-gray-600'))))) }}">
                                                        @include('livewire.hr.partials.file-icon', ['icon' => $file['icon']])
                                                    </div>
                                                @endif
                                                <span class="text-xs font-medium text-gray-700 line-clamp-2 group-hover:text-brand-600">{{ $file['name'] }}</span>
                                                @if ($file['size'])
                                                    <span class="text-[10px] text-gray-600 mt-0.5">{{ number_format($file['size'] / 1024, 0) }} KB</span>
                                                @endif
                                            </button>
                                        @endif
                                    @endforeach
                                ...
                                <div class="divide-y divide-gray-100">
                                    @foreach ($files as $file)
                                        <div class="flex items-center gap-3 py-2 px-2 hover:bg-gray-50 rounded-lg transition {{ $file['isFolder'] ? 'cursor-pointer' : '' }}"
                                            ...
                                            {{-- Icon --}}
                                            <div class="flex-shrink-0 w-10 h-10 flex items-center justify-center {{ $file['isFolder'] ? 'text-warning-500' : ($file['icon'] === 'pdf' ? 'text-danger-500' : ($file['icon'] === 'doc' ? 'text-blue-500' : ($file['icon'] === 'sheet' ? 'text-success-500' : ($file['icon'] === 'slides' ? 'text-yellow-500' : ($file['icon'] === 'image' ? 'text-purple-500' : ($file['icon'] === 'video' ? 'text-pink-500' : 'text-gray-600')))))) }}">
                                                @if ($file['isFolder'])
                                            ...
                                            <div class="flex-1 min-w-0">
                                                @if ($file['isFolder'])
                                                    <span class="text-sm font-medium text-gray-800 hover:text-brand-600">{{ $file['name'] }}</span>
                                                @else
                                                    <button wire:click="openPreview('{{ $file['id'] }}')" class="text-sm font-medium text-gray-800 hover:text-brand-600 truncate block w-full text-left">
                                                        {{ $file['name'] }}
                                                    </button>
                                                @endif
                                            </div>
                                            ...
                                            {{-- Size --}}
                                            <div class="hidden sm:block text-xs text-gray-600 w-20 text-right">
                                                @if ($file['size'])
                                            ...
                                            @if (!$file['isFolder'])
                                                <div class="flex items-center gap-1">
                                                    <button wire:click="openPreview('{{ $file['id'] }}')" class="p-1.5 text-gray-600 hover:text-brand-600 rounded" title="Preview">
                                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                                    </button>
                                                    <a href="https://drive.google.com/uc?export=download&id={{ $file['id'] }}" target="_blank" class="p-1.5 text-gray-600 hover:text-success-600 rounded" title="Download">
                ...
                <div class="relative bg-white w-full h-full flex flex-col" @click.stop>
                    ...
                    <div class="flex items-center justify-between px-4 py-2 bg-gray-900 text-white flex-shrink-0">
                        <div class="flex items-center gap-3 min-w-0">
                            <div class="flex-shrink-0 {{ $previewFile['icon'] === 'pdf' ? 'text-danger-400' : ($previewFile['icon'] === 'doc' ? 'text-blue-400' : ($previewFile['icon'] === 'sheet' ? 'text-success-400' : 'text-gray-400')) }}">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            @if ($previewFile['isPresentation'])
                                <a href="https://docs.google.com/presentation/d/{{ $previewFile['id'] }}/present"
                                   target="_blank"
                                   class="px-3 py-1.5 text-xs font-medium text-white bg-brand-600 rounded-lg hover:bg-brand-700 transition flex items-center gap-1"
                            ...
                            <a href="https://drive.google.com/uc?export=download&id={{ $previewFile['id'] }}"
                               target="_blank"
                               class="px-3 py-1.5 text-xs font-medium text-white bg-success-600 rounded-lg hover:bg-success-700 transition flex items-center gap-1">
                    ...
                    <div class="flex-1 overflow-hidden bg-gray-900 relative">
                        ...
                        @else
                            <div class="h-full flex flex-col items-center justify-center text-gray-400 p-8">
                                ...
                                <a href="https://drive.google.com/uc?export=download&id={{ $previewFile['id'] }}"
                                   target="_blank"
                                   class="px-6 py-3 text-sm font-medium text-white bg-brand-600 rounded-lg hover:bg-brand-700 transition">
                                    Download File
                                </a>
                            </div>
                        @endif
