            ...
            <tbody class="divide-y divide-gray-50">
                @forelse ($invoices as $inv)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-center {{ $inv->due_date?->isPast() && $inv->status !== 'paid' ? 'text-danger-500 font-medium' : 'text-gray-500' }}">
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium
                                {{ match($inv->status) { 'issued' => 'bg-blue-100 text-blue-700', 'partial' => 'bg-warning-100 text-warning-700', 'paid' => 'bg-success-100 text-success-700', 'overdue' => 'bg-danger-100 text-danger-600', default => 'bg-gray-100 text-gray-600' } }}">
                                {{ $inv->status === 'partial' ? 'Partially Paid' : ucfirst($inv->status) }}
                            </span>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-gray-600">No invoices found.</td></tr>
                @endforelse
            </tbody>
        </table>
