            ...
            <tbody>
                @forelse ($orders as $po)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            <a href="{{ route('supplier.orders.show', $po->id) }}" class="text-brand-600 hover:underline font-mono text-xs font-medium">{{ $po->po_number }}</a>
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium
                                {{ match($po->status) { 'approved' => 'bg-brand-100 text-brand-700', 'sent' => 'bg-blue-100 text-blue-700', 'partial' => 'bg-orange-100 text-orange-700', 'received' => 'bg-success-100 text-success-700', default => 'bg-gray-100 text-gray-600' } }}">
                                {{ ucfirst($po->status) }}
                            </span>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-gray-600">No orders found.</td></tr>
                @endforelse
            </tbody>
        </table>
        @if ($orders->hasPages())
