<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="card p-5">
            <p class="text-xs text-gray-600 uppercase tracking-wider">Pending Orders</p>
            <p class="text-2xl font-bold text-gray-800 mt-1">{{ $pendingPos }}</p>
        </div>
        <div class="card p-5">
            <p class="text-xs text-gray-600 uppercase tracking-wider">Total Orders</p>
            <p class="text-2xl font-bold text-gray-800 mt-1">{{ $totalPos }}</p>
        </div>
        <div class="card p-5">
            <p class="text-xs text-gray-600 uppercase tracking-wider">Active Products</p>
            <p class="text-2xl font-bold text-gray-800 mt-1">{{ $activeProducts }}</p>
        </div>
        <div class="card p-5">
            <p class="text-xs text-gray-600 uppercase tracking-wider">Outstanding Amount</p>
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse ($recentOrders as $po)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            <a href="{{ route('supplier.orders.show', $po->id) }}" class="text-brand-600 hover:underline font-mono text-xs">{{ $po->po_number }}</a>
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium
                                {{ match($po->status) { 'approved' => 'bg-brand-100 text-brand-700', 'sent' => 'bg-blue-100 text-blue-700', 'received' => 'bg-success-100 text-success-700', default => 'bg-gray-100 text-gray-600' } }}">
                                {{ ucfirst($po->status) }}
                            </span>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="px-4 py-8 text-center text-gray-600">No orders yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
