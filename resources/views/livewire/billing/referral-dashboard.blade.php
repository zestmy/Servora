<div>
    ...
    <p class="text-xs text-gray-600 mb-6">Share Servora with other F&B businesses and earn commission on every signup.</p>
                ...
                @if ($referralCode)
                    ...
                    @if ($referralStats)
                        <div class="grid grid-cols-4 gap-3 mt-5 pt-5 border-t border-gray-100">
                            ...
                            <div class="text-center">
                                <p class="text-xl font-bold text-success-600">{{ number_format($referralStats['conversions']) }}</p>
                                <p class="text-[10px] text-gray-500 uppercase tracking-wider font-medium">Converted</p>
                            </div>
                            <div class="text-center">
                                <p class="text-xl font-bold text-brand-600">RM {{ number_format($referralStats['earned'], 2) }}</p>
                                ...
                                @foreach ($referralStats['referrals'] as $ref)
                                    <div class="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg">
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-600">{{ $ref->created_at->format('d M Y') }}</p>
                                        </div>
                ...
                @else
                    ...
                    <div class="text-center py-6">
                        <div class="w-14 h-14 bg-brand-50 rounded-full flex items-center justify-center mx-auto mb-4">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="space-y-4">
                    <div class="flex items-start gap-3">
                        <div class="w-6 h-6 bg-brand-100 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                            <span class="text-xs font-bold text-brand-600">1</span>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-700">Share your link</p>
                            <p class="text-xs text-gray-600">Send your referral link to other F&B business owners</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        <div class="w-6 h-6 bg-brand-100 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                            <span class="text-xs font-bold text-brand-600">2</span>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-700">They sign up</p>
                            <p class="text-xs text-gray-600">When they register using your link, it's tracked automatically</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        <div class="w-6 h-6 bg-brand-100 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                            <span class="text-xs font-bold text-brand-600">3</span>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-700">Earn commission</p>
                            <p class="text-xs text-gray-600">When they subscribe, you earn commission on their payment</p>
                        </div>
                    </div>
                </div>
            </div>

            <a href="{{ route('referral.program') }}" class="block mt-4 text-center text-xs text-brand-600 hover:text-brand-800 font-medium transition">
