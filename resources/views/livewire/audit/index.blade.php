<div class="max-w-7xl mx-auto px-4 py-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
            ...
            <p class="text-xs text-gray-600 mt-0.5">Every tracked action across your operation — who did what, and when.</p>
        </div>
    ...
    <div class="card p-4 mb-4">
        ...
        <div class="flex flex-wrap gap-3 items-center">
            ...
            {{-- User --}}
            <select wire:model.live="userFilter" class="rounded-lg border-gray-300 text-sm shadow-sm focus:border-brand-500 focus:ring-brand-500">
            ...
            {{-- Module --}}
            <select wire:model.live="typeFilter" class="rounded-lg border-gray-300 text-sm shadow-sm focus:border-brand-500 focus:ring-brand-500">
            ...
            {{-- Event --}}
            <select wire:model.live="eventFilter" class="rounded-lg border-gray-300 text-sm shadow-sm focus:border-brand-500 focus:ring-brand-500">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search user / record #…"
                   class="rounded-lg border-gray-300 text-sm shadow-sm focus:border-brand-500 focus:ring-brand-500 min-w-[200px]" />

            <button wire:click="resetFilters" class="text-xs text-gray-600 hover:text-gray-900 underline">Reset</button>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="table-surface min-w-[900px]">
                ...
                <tbody>
                    @forelse ($logs as $log)
                        @php
                            $badge = match ($log->event) {
                                ...
                                'deleted', 'force_deleted'    => 'bg-danger-100 text-danger-600',
                                'restored'                    => 'bg-brand-100 text-brand-700',
                                'approved', 'received', 'submitted' => 'bg-success-100 text-success-700',
                                'rejected', 'cancelled'       => 'bg-danger-100 text-danger-600',
                        ...
                        <tr x-data="{ open: false }" wire:key="log-{{ $log->id }}" class="hover:bg-gray-50 transition align-top">
                            ...
                            <td class="px-4 py-3 text-gray-700">{{ $log->actorName() }}@if($log->guard === 'lms')<span class="text-[10px] text-gray-600"> (LMS)</span>@endif</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                @if ($recLabel = $recordLabels[$log->auditable_type . ':' . $log->auditable_id] ?? null)
                                    ...
                                    <span class="text-[11px] text-gray-600 whitespace-nowrap">#{{ $log->auditable_id }}</span>
                                @else
                            ...
                            <td class="px-4 py-3 text-right">
                                @if (!empty($keys))
                                    <button @click="open = !open" class="text-xs text-brand-600 hover:text-brand-700">
                        ...
                        @if (!empty($keys))
                            <tr x-show="open" x-cloak wire:key="log-detail-{{ $log->id }}" class="bg-gray-50/60">
                                <td colspan="7" class="px-4 py-3">
                                            ...
                                            <tbody>
                                                @foreach ($keys as $k)
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="px-3 py-1.5 text-danger-600 break-all">{{ $fmt($before) }}</td>
                                                        <td class="px-3 py-1.5 text-success-700 break-all">{{ $fmt($after) }}</td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
                                    ...
                                    @if ($log->ip_address)
                                        <p class="text-[10px] text-gray-600 mt-1.5">IP {{ $log->ip_address }}</p>
                                    @endif
                                </td>
                            </tr>
                        @endif
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-10 text-center text-sm text-gray-600">No audit entries match these filters.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
