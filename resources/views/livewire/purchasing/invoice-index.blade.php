<div>
    ...
    <div class="card overflow-hidden">
        ...
        <div class="md:hidden divide-y divide-gray-100">
            @forelse ($invoices as $inv)
                ...
                <div class="p-3 space-y-2">
                    <div class="flex items-start justify-between gap-2">
                        <a href="{{ route('purchasing.invoices.show', $inv->id) }}" wire:navigate class="font-mono text-sm font-medium text-brand-600">{{ $inv->invoice_number }}</a>
                    ...
                    <div class="flex items-center gap-2">
                        <span class="px-2 py-0.5 rounded text-[10px] {{ $inv->type === 'supplier' ? 'bg-purple-50 text-purple-600' : 'bg-warning-50 text-warning-600' }}">
                    ...
                    <div class="flex items-center justify-between text-xs text-gray-500">
                        <div class="flex items-center gap-3">
                            ...
                            @if ($inv->due_date)
                                <span class="{{ $mDueRed ? 'text-danger-500 font-medium' : 'text-gray-500' }}">Due {{ $inv->due_date->format('d M') }}</span>
                            @endif
                        </div>
                        <span class="tabular-nums font-semibold text-gray-900">
                            RM {{ number_format($inv->total_amount, 2) }}
                            @if ($inv->status === 'partial')
                                <span class="block text-[10px] font-medium text-warning-600 text-right">Bal: {{ number_format($inv->balance_due, 2) }}</span>
                            @endif
                        </span>
                    </div>
            ...
            @empty
                <div class="p-8 text-center text-gray-600 text-sm font-medium">No invoices found.</div>
            @endforelse
        </div>
        ...
        <table class="hidden md:table min-w-full divide-y divide-gray-100 text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse ($invoices as $inv)
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 font-mono text-xs font-medium">
                            <a href="{{ route('purchasing.invoices.show', $inv->id) }}" wire:navigate class="text-brand-600 hover:text-brand-800 hover:underline">{{ $inv->invoice_number }}</a>
                        </td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-0.5 rounded text-xs {{ $inv->type === 'supplier' ? 'bg-purple-50 text-purple-600' : 'bg-warning-50 text-warning-600' }}">
                                {{ $inv->type === 'supplier' ? 'Supplier' : 'CPU → Outlet' }}
                            </span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            @if ($inv->due_date)
                                <span class="{{ $inv->due_date->isPast() && $inv->status !== 'paid' ? 'text-danger-500 font-medium' : 'text-gray-500' }}">
                        ...
                        <td class="px-4 py-3 text-right tabular-nums font-medium text-gray-800">
                            ...
                            @if ($inv->status === 'partial')
                                <span class="block text-[10px] font-medium text-warning-600">Bal: {{ number_format($inv->balance_due, 2) }}</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-center">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1">
                                @if (in_array($inv->status, ['issued', 'partial', 'overdue']))
                                    <button wire:click="markPaid({{ $inv->id }})" wire:confirm="Record full payment and mark as paid?"
                                            title="Mark Paid" class="text-success-500 hover:text-success-700 transition p-1">
                ...
                @empty
                    <tr><td colspan="9" class="px-4 py-8 text-center text-gray-600">No invoices found.</td></tr>
                @endforelse
            </tbody>
        </table>
