<div>
    ...
    <div class="flex items-center gap-3 mb-6">
        <a data-back href="{{ route('purchasing.invoices.index') }}" class="text-gray-600 hover:text-gray-900 transition flex-shrink-0">
        ...
        <div class="flex-1 min-w-0">
            ...
            <h2 class="page-title">AI Invoice Receiving</h2>
        </div>
    </div>
            ...
            <div class="card p-8">
                <div class="text-center mb-6">
                    <div class="w-16 h-16 mx-auto bg-brand-50 rounded-full flex items-center justify-center mb-4">
                        <svg class="w-8 h-8 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                    </div>
                    <h3 class="page-title">Upload Supplier Invoice</h3>
                    ...
                    <div x-data="{ dragover: false }"
                        ...
                        @if ($invoiceFile)
                            <div class="text-sm text-gray-700">
                                <svg class="w-8 h-8 mx-auto text-success-500 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                <p class="font-medium">{{ $invoiceFile->getClientOriginalName() }}</p>
                                <p class="text-xs text-gray-600 mt-1">{{ number_format($invoiceFile->getSize() / 1024, 0) }} KB</p>
                            </div>
                        @else
                            <svg class="w-10 h-10 mx-auto text-gray-600 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
                            <p class="text-sm text-gray-600">Drop file here or click to browse</p>
                            <p class="text-xs text-gray-600 mt-1">JPG, PNG, WebP, or PDF — max 10 MB</p>
                        @endif
                    </div>

                    @error('invoiceFile')
                        <p class="text-danger-500 text-xs mb-3">{{ $message }}</p>
                    @enderror
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
            ...
            <div class="lg:col-span-3 space-y-4">
                ...
                @if (count($exceptions) > 0)
                    <div class="card p-4">
                        <div class="flex items-center gap-3">
                            @if ($errorCount > 0)
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-danger-100 text-danger-700">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            @if ($supplierConfidence > 0 && $supplierConfidence < 1)
                                <p class="text-xs mt-0.5 {{ $supplierConfidence >= 0.8 ? 'text-success-600' : 'text-warning-600' }}">
                            ...
                            @endif
                            @error('selectedSupplierId') <p class="text-danger-500 text-xs mt-0.5">{{ $message }}</p> @enderror
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            @error('issuedDate') <p class="text-danger-500 text-xs mt-0.5">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            ...
                            <tbody>
                                @foreach ($lines as $i => $line)
                                    @php
                                        ...
                                        $rowBg = $hasError ? 'bg-danger-50' : ($hasWarning ? 'bg-warning-50' : ($line['match_confidence'] >= 0.5 ? 'bg-success-50/30' : 'bg-gray-50/50'));
                                    @endphp
                                    <tr class="{{ $rowBg }}">
                                        <td class="px-3 py-2 text-gray-600">
                                            {{ $i + 1 }}
                                            @if ($hasError)
                                                <span class="inline-block w-2 h-2 rounded-full bg-danger-500 ml-0.5" title="{{ $lineExceptions->where('severity', 'error')->pluck('message')->join('; ') }}"></span>
                                            @elseif ($hasWarning)
                                                <span class="inline-block w-2 h-2 rounded-full bg-warning-500 ml-0.5" title="{{ $lineExceptions->where('severity', 'warning')->pluck('message')->join('; ') }}"></span>
                                            @endif
                                        </td>
                                        <td class="px-3 py-2">
                                            ...
                                            @if ($line['match_confidence'] > 0 && $line['match_confidence'] < 0.5)
                                                <p class="text-xs text-warning-500 mt-0.5">Not sure about this product — please check it's the right one.</p>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-3 py-2 text-center">
                                            @if ($line['po_unit_price'] !== null)
                                                <span class="text-xs tabular-nums text-gray-600">{{ number_format($line['po_unit_price'], 2) }}</span>
                                            @else
                                        ...
                                        <td class="px-3 py-2 text-center">
                                            <button wire:click="removeLine({{ $i }})" wire:confirm="Remove this line?"
                                                    class="text-gray-600 hover:text-danger-500 transition" title="Remove">
                ...
                <div class="flex items-center gap-3">
                    <button wire:click="approve" wire:confirm="Create invoice from this scanned data?"
                            class="px-6 py-2.5 bg-success-600 text-white font-medium rounded-lg hover:bg-success-700 transition inline-flex items-center gap-2">
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                <div class="card p-4">
                    ...
                    @if ($selectedPoId)
                        <div class="space-y-2 mb-3">
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-mono text-xs font-medium text-brand-600">{{ $matchedPoNumber }}</span>
                            </div>
                            @if ($poConfidence > 0)
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-500">Confidence</span>
                                    <span class="text-xs {{ $poConfidence >= 0.7 ? 'text-success-600' : ($poConfidence >= 0.4 ? 'text-warning-600' : 'text-danger-600') }}">
                    ...
                    @else
                        <p class="text-sm text-gray-600">No matching PO found</p>
                ...
                @if ($selectedGrnId)
                    <div class="card p-4">
                        ...
                        <span class="font-mono text-xs font-medium text-brand-600">{{ $matchedGrnNumber }}</span>
                    </div>
                @endif
                ...
                @if (count($exceptions) > 0)
                    <div class="card p-4">
                        ...
                        <p class="text-[11px] text-gray-600 mb-3">Differences we spotted between this invoice and your records — review each before approving.</p>
