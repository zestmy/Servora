<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="page-title">Convert PO to Delivery Order</h2>
            <p class="text-xs text-gray-600 mt-0.5">Review items and create DO for delivery arrangement</p>
        </div>
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="card p-5">
                ...
                <div class="grid grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                        <p class="text-gray-600 text-xs">Outlet</p>
                        ...
                        <div class="absolute z-20 mt-1 w-full bg-white rounded-lg shadow-lg border border-gray-200 max-h-48 overflow-y-auto">
                            @foreach ($searchResults as $ing)
                                <button type="button" wire:click="addIngredient({{ $ing->id }})"
                                        class="w-full text-left px-4 py-2 text-sm hover:bg-brand-50 transition">
                                    <span class="font-medium text-gray-800">{{ $ing->name }}</span>
                                    <span class="text-gray-600 ml-2">{{ $ing->baseUom?->abbreviation }}</span>
                                </button>
                            @endforeach
                        </div>
                    @endif
                        ...
                        <tbody>
                            @forelse ($lines as $idx => $line)
                                <tr class="border-b border-gray-50">
                                    ...
                                    <td class="py-2 px-2 text-center text-gray-600 text-xs">
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ $showPrice ? 7 : 5 }}" class="py-8 text-center text-gray-600 text-sm">No items added</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
