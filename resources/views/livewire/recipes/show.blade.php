<div>
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div class="flex-1 min-w-0">
            <p class="text-xs text-gray-600"><a href="{{ route('recipes.index') }}" class="hover:underline">Recipes</a> / {{ $recipe->name }}</p>
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="card p-6">
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <p class="mt-0.5">
                            <span class="inline-flex px-2 py-0.5 text-xs font-semibold rounded-full {{ $recipe->is_active ? 'bg-success-100 text-success-700' : 'bg-gray-100 text-gray-500' }}">
            ...
            <div class="card overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100">
                    ...
                    <p class="text-xs text-gray-600 mt-0.5">{{ count($lineData) }} item{{ count($lineData) !== 1 ? 's' : '' }}</p>
                </div>
                                ...
                                @foreach ($lineData as $idx => $ld)
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        <td class="px-4 py-2">
                                            ...
                                            @if ($ld['is_prep'])
                                                <span class="ml-1 px-1.5 py-0.5 bg-warning-100 text-warning-700 text-xs font-semibold rounded">PREP</span>
                                            @endif
                                        </td>
                                        ...
                                        <tr>
                                            <td colspan="6" class="px-4 py-1.5 text-right text-sm text-gray-500">
                                                ...
                                                @if (($ec['type'] ?? 'value') === 'percent')
                                                    <span class="text-xs text-gray-600">({{ $ec['amount'] }}%)</span>
                                                @endif
                                            </td>
        ...
        <div class="lg:col-span-1 space-y-4">
            ...
            <div class="card p-6">
                    ...
                    <div class="space-y-3">
                        @foreach ($pricingAnalysis as $pa)
                            @if ($pa['selling_price'] > 0)
                                @php
                                    $fcColor = match(true) {
                                        $pa['food_cost_pct'] === null => 'text-gray-600',
                                        $pa['food_cost_pct'] <= 25    => 'text-success-600',
                                    ...
                                    $fcBg = match(true) {
                                        ...
                                        $pa['food_cost_pct'] <= 25    => 'bg-success-50',
                                ...
                                <div class="rounded-lg {{ $fcBg }} px-3 py-3">
                                    <div class="flex justify-between items-center mb-1">
                                        <span class="text-sm font-medium text-gray-700">
                                            {{ $pa['name'] }}
                                            @if ($pa['is_default'])
                                                <span class="text-xs text-brand-600">(Default)</span>
                                            @endif
                                        </span>
                                    ...
                                    <div class="flex justify-between text-xs mt-0.5">
                                        ...
                                        <span class="{{ $pa['gross_profit'] >= 0 ? 'text-success-600' : 'text-danger-600' }} font-medium tabular-nums">
                ...
                @elseif ($legacyPrice > 0)
                    @php
                        $fcColor = match(true) {
                            $legacyFoodCostPct === null => 'text-gray-600',
                            $legacyFoodCostPct <= 25    => 'text-success-600',
                    ...
                    <div class="rounded-lg bg-gray-50 px-3 py-3">
                        ...
                        <div class="flex justify-between text-xs mt-0.5">
                            <span class="text-gray-500">Gross Profit</span>
                            <span class="{{ ($legacyPrice - $grandCost) >= 0 ? 'text-success-600' : 'text-danger-600' }} font-medium tabular-nums">
                                {{ number_format($legacyPrice - $grandCost, 2) }}
                            </span>
                        </div>
                    </div>
                @else
                    <p class="text-xs text-gray-600 italic">No selling prices set.</p>
                @endif

                {{-- Benchmark guide --}}
                <div class="text-xs text-gray-600 space-y-0.5 pt-3 mt-3 border-t border-gray-100">
