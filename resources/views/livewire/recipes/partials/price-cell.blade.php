    ...
    @else
        <div x-data="{ editing: false, val: @js($display), orig: @js($display), cancel: false }" class="flex justify-end">
            <button type="button" x-show="! editing"
                    ...
                    class="group inline-flex items-center gap-1 hover:text-brand-600 transition"
                    title="Click to edit price">
                <span x-text="val !== '' ? Number(val).toFixed(2) : '—'"
                      :class="val === '' ? 'text-gray-500 group-hover:text-brand-400' : ''"></span>
                <svg class="w-3 h-3 text-gray-500 opacity-0 group-hover:opacity-100 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <input x-show="editing" x-cloak x-ref="inp" x-model="val"
                   ...
                   class="w-24 text-right rounded border-gray-300 text-sm py-0.5 px-1.5 focus:border-brand-500 focus:ring-brand-500"
