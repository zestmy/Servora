<div>
    ...
    <div class="flex items-center gap-3 mb-6">
        <a data-back href="{{ route('recipes.index') }}" class="text-gray-600 hover:text-gray-900 transition flex-shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
            </svg>
        </a>
        <div class="flex-1 min-w-0">
            <p class="text-xs text-gray-600"><a href="{{ route('recipes.index') }}" class="hover:underline">Recipes</a> / {{ $recipeId ? $name : 'New Recipe' }}</p>
        </div>
        @if ($recipeId)
            ...
            <div class="card p-6 space-y-4">
                    ...
                    @if ($priceClasses->isEmpty())
                        <div>
                            ...
                            <x-text-input id="r_price" wire:model.live="selling_price"
                                          ...
                                          class="mt-1 block w-full" />
                            <p class="mt-0.5 text-xs text-gray-600">Per {{ $yield_quantity ?: '1' }} {{ collect($uoms)->firstWhere('id', $yield_uom_id)?->abbreviation ?? 'serving' }}</p>
                            ...
                            @foreach ($priceClasses as $pc)
                                <div>
                                    <label class="text-xs font-medium text-gray-600 flex items-center gap-1">
                                        {{ $pc->name }}
                                        @if ($pc->is_default)
                                            <span class="px-1 py-0.5 bg-brand-100 text-brand-600 text-xs rounded">Default</span>
                                        @endif
                                    </label>
                ...
                @if ($outlets->count() > 1)
                    <div class="border-t border-gray-100 pt-4">
                        ...
                        <p class="text-xs text-gray-600 mt-0.5 mb-3">Tag this recipe to specific outlets, or leave as "All Outlets" to make it available everywhere.</p>
                        ...
                        @if ($ingredient->is_active)
                            <button type="button"
                                ...
                                <div class="text-right flex-shrink-0 ml-4">
                                    ...
                                    <span class="ml-2 text-xs text-brand-400">+ Add</span>
                                </div>
                            </button>
                        @else
                                ...
                                <td class="px-4 py-2">
                                    <div class="relative">
                                        <input type="number" step="0.1" min="0" max="100"
                                               wire:model.live.debounce.400ms="lines.{{ $idx }}.waste_percentage" wire:loading.attr="disabled" wire:target="reorderLines,reorderPackagingLines"
                                               class="w-full text-right pr-7 rounded border-gray-300 text-sm focus:border-brand-500 focus:ring-brand-500" />
                                        <span class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-600 text-xs pointer-events-none">%</span>
                                    </div>
        ...
        <div class="border-t border-gray-100">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <div>
                    ...
                    <p class="text-xs text-gray-600 mt-0.5">{{ count($packagingLines) }} item{{ count($packagingLines) !== 1 ? 's' : '' }} · counted separately so packaging cost appears as its own line</p>
                </div>
            </div>
            ...
            <div class="px-6 py-4 border-b border-gray-100">
                ...
                @elseif (strlen($packagingSearch) >= 2)
                    <p class="mt-2 text-sm text-gray-600 text-center py-2">No items found for "{{ $packagingSearch }}".</p>
                @endif
            </div>
            ...
            @if (count($packagingLines))
                <div class="overflow-x-auto">
                    <table class="table-surface min-w-full">
                        ...
                        <tbody
                               x-data
                               x-init="window.sortableRows($el, { value: (row) => row.dataset.packIdx, selector: 'tr[data-pack-idx]', commit: (order) => $wire.reorderPackagingLines(order) })" wire:loading.class="opacity-60" wire:target="reorderLines,reorderPackagingLines">
        ...
        <div class="px-6 py-4 border-t border-gray-100">
            <div class="flex items-center justify-between mb-3">
                <div>
                    ...
                    <p class="text-xs text-gray-600 mt-0.5">Add overhead costs like packaging, electricity, logistics, labour.</p>
                </div>
                    ...
                    @foreach ($extraCosts as $idx => $cost)
                        <div class="flex items-center gap-2" wire:key="extra-cost-{{ $idx }}">
                            ...
                            @if (($cost['type'] ?? 'value') === 'percent' && floatval($cost['amount'] ?? 0) > 0 && $totalCost > 0)
                                <span class="text-xs text-gray-600 tabular-nums w-20 text-right">= {{ number_format($totalCost * floatval($cost['amount']) / 100, 2) }}</span>
                            @else
    ...
    {{-- ── Product Images ── --}}
    <div class="mt-4 card p-6" x-data="{ imageTab: 'dine_in' }">
            ...
            @if (count($newDineInImages))
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 mt-3">
                    @foreach ($newDineInImages as $idx => $file)
                        <div class="relative group rounded-lg overflow-hidden border border-brand-200 bg-brand-50">
        ...
        <div x-show="imageTab === 'takeaway'" x-cloak>
            ...
            <div>
                <label class="block">
                    <div class="flex items-center justify-center w-full h-24 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-warning-400 hover:bg-warning-50/30 transition">
                        <div class="text-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto h-7 w-7 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            </svg>
            ...
            @if (count($newTakeawayImages))
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 mt-3">
                    @foreach ($newTakeawayImages as $idx => $file)
                        <div class="relative group rounded-lg overflow-hidden border border-warning-200 bg-warning-50">
                            <img src="{{ $file->temporaryUrl() }}" alt="New upload"
                                 class="w-full h-32 object-cover" />
                            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                            ...
                            <div class="absolute top-1 right-1 px-1.5 py-0.5 bg-warning-600 text-white text-xs rounded font-medium">New</div>
                            <div class="px-2 py-1.5 text-xs text-gray-500 truncate">{{ $file->getClientOriginalName() }}</div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="card p-6">
                    ...
                    <dl class="space-y-3 text-sm">
                        ...
                        @if ($totalTaxWithPackaging > 0)
                            <div class="flex justify-between">
                                <dt class="text-gray-500">
                                    Cost / {{ $yieldUomAbbr }} <span class="text-xs text-gray-600">(w/ tax)</span>
                                </dt>
                                <dd class="text-brand-700 tabular-nums">{{ number_format($costPerServingWithTax, 4) }}</dd>
                            </div>
                        @endif
                        ...
                        @if ($priceClasses->isNotEmpty())
                            ...
                            <div class="border-t border-gray-100 pt-3 space-y-2">
                                ...
                                @foreach ($priceClasses as $pc)
                                    @php
                                        ...
                                        $pcColor = match(true) {
                                            $fcp === null => 'text-gray-600',
                                            $fcp <= 25    => 'text-success-600',
                                    ...
                                    @if ($sp > 0)
                                        <div class="rounded-lg bg-gray-50 px-3 py-2">
                                            ...
                                            <div class="flex justify-between mt-0.5 text-xs">
                                                ...
                                                <span class="{{ $gp >= 0 ? 'text-success-600' : 'text-danger-600' }} font-medium tabular-nums">{{ number_format($gp, 2) }}</span>
                                            </div>
                                        </div>
                                    @endif
                                @endforeach
                                @if (collect($classCostData)->every(fn ($cd) => ($cd['selling_price'] ?? 0) <= 0))
                                    <div class="text-xs text-gray-600 italic">Enter selling prices above to see food cost %.</div>
                                @endif
                            </div>
                            ...
                            <div class="text-xs text-gray-600 space-y-0.5 pt-1">
                                ...
                                <p><span class="text-success-600">≤25%</span> Excellent &nbsp;
                                   <span class="text-yellow-600">25–35%</span> Good</p>
                                <p><span class="text-orange-500">35–45%</span> High &nbsp;
                                   <span class="text-danger-600">&gt;45%</span> Review</p>
                            </div>
                        @elseif (floatval($selling_price) > 0)
                            ...
                            @php
                                $fcColor = match(true) {
                                    $foodCostPct === null => 'text-gray-600',
                                    $foodCostPct <= 25   => 'text-success-600',
                            ...
                            <div class="rounded-lg {{ $fcBg }} px-3 py-2">
                                ...
                                @if ($totalTaxWithPackaging > 0 && $foodCostPctWithTax !== null)
                                    <div class="flex justify-between mt-1 text-xs">
                                        ...
                                        <span class="font-medium text-brand-600 tabular-nums">{{ number_format($foodCostPctWithTax, 1) }}%</span>
                                    </div>
                                @endif
                                <div class="flex justify-between mt-1 text-xs">
                                    <span class="text-gray-500">Gross Profit</span>
                                    <span class="{{ $grossProfit >= 0 ? 'text-success-600' : 'text-danger-600' }} font-medium tabular-nums">
