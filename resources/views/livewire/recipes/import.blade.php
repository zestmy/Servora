<div>
    ...
    <div class="flex items-center gap-3 mb-6">
        <a data-back href="{{ route('recipes.index', $isPrep ? ['tab' => 'prep-items'] : []) }}" class="text-gray-600 hover:text-gray-900 transition flex-shrink-0">
                    ...
                    <tbody>
                        @foreach ($rows as $row)
                            <tr class="{{ $row['skip'] ? 'bg-danger-50' : 'hover:bg-gray-50' }} transition">
                                ...
                                <td class="px-3 py-2 text-gray-500 max-w-xs truncate">{{ $row['description'] ?? '—' }}</td>
                                <td class="px-3 py-2 text-right tabular-nums text-gray-700">{{ $row['yield_quantity'] }}</td>
                                <td class="px-3 py-2 text-gray-600 font-mono">{{ $row['yield_uom_label'] ?: '—' }}</td>
