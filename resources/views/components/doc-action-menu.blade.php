...
<div x-data="{ open: false, share: false }" class="relative inline-block" @click.outside="open = false; share = false">
    {{-- Kebab trigger — filled circles for visibility --}}
    ...
    <div x-show="open" x-cloak
         x-anchor.bottom-end.offset.4="$refs?.trigger ?? $el.previousElementSibling"
         ...
         class="w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 text-xs">
        ...
        {{-- Share — inline options (no sub-dropdown to avoid clipping) --}}
        ...
        {{-- Copy Link --}}
        <button type="button"
                @click="navigator.clipboard.writeText('{{ $pdfLink ?: request()->url() }}'); open = false; $dispatch('notify', { message: 'Link copied!' })"
