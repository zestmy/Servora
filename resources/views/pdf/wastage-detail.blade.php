...
@section('content')
    ...
    <div class="header">
        ...
        <div class="header-right">
            <div class="doc-title">Wastage Details</div>
            <div class="doc-number">{{ \Carbon\Carbon::parse($scope['from'])->format('d M Y') }} &ndash;
                {{ \Carbon\Carbon::parse($scope['to'])->format('d M Y') }}</div>
            <div class="doc-status">{{ $report['records']->count() }} {{ \Illuminate\Support\Str::plural('note', $report['records']->count()) }} merged</div>
        </div>
    </div>
    ...
    @if ($report['itemCount'] === 0)
        <div style="border: 1px solid #e5e7eb; padding: 24px; text-align: center; color: #64748b; font-size: 10pt;">
            No wastage recorded in this range under these filters.
        </div>
    @else
        {{-- Items --}}
        <table class="items">
            <thead>
                <tr>
                    ...
                    <th class="right" style="width: 60px;">Unit Cost</th>
                    <th class="right" style="width: 68px;">Value (RM)</th>
                    <th>Reason(s)</th>
                </tr>
            </thead>
            <tbody>
                    ...
                    @foreach ($group['items'] as $item)
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @endif
                                @if ($item['lines'] > 1)
                                    <span style="color: #888; font-size: 8px;">({{ $item['lines'] }} notes)</span>
                                @endif
                            </td>
                            ...
                            <td style="font-size: 8pt; color: #475569;">{{ implode(', ', $item['reasons']) ?: '—' }}</td>
                        </tr>
                    @endforeach
                @endforeach
                <tr>
                    <td colspan="5" style="text-align: right; font-weight: bold; padding: 7px 8px; border-top: 2px solid #475569;">Total wastage value</td>
                    <td colspan="2" class="right" style="font-weight: bold; padding: 7px 8px; border-top: 2px solid #475569;">
    ...
    {{-- What was merged. The point of filing this is being able to answer
         "which notes is that number from?" a year later. --}}
    <div style="margin-top: 18px; page-break-inside: avoid;">
        <h4 style="font-size: 9pt; font-weight: bold; text-transform: uppercase; letter-spacing: 1px; color: #475569; margin-bottom: 6px;">Notes merged</h4>
        <table class="items">
            <thead>
                <tr>
            ...
            <tbody>
                @forelse ($report['records'] as $i => $record)
                    <tr>
                        <td>{{ $i + 1 }}</td>
                        <td>{{ $record->wastage_date?->format('d M Y') ?? '—' }}</td>
                        <td>{{ $record->reference_number ?? 'W-' . $record->id }}</td>
