...
@section('content')
    ...
    @if (! empty($activeFilters))
        <div style="background: #eff6ff; border-left: 3px solid #2563eb; padding: 5px 10px; margin-bottom: 8px; font-size: 9pt; color: #1d4ed8;">
    ...
    <table class="items">
        <thead>
            <tr>
                ...
                <th class="right" style="width: 60px;">Cost/Unit</th>
                @if (! ($isPrep ?? false))
        ...
        <tbody>
            ...
            @foreach ($summaryRows as $idx => $row)
                @if ($row['category'] !== $currentCategory)
                    ...
                    @if ($currentCategory)
                        <tr>
                            <td colspan="{{ ($isPrep ?? false) ? 8 : ($priceClasses->isNotEmpty() ? 8 + ($priceClasses->count() * 2) : 10) }}"
                ...
                <tr>
                    ...
                    <td class="right" style="color: {{ ($row['packaging_cost'] ?? 0) > 0 ? '#1d4ed8' : '#ccc' }};">
